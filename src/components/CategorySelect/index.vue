     ...
     -->
        <el-form :inline="true" class="demo-form-inline" :model="cateForm">
            <el-form-item label="一级分类">
                <el-select placeholder="请选择" v-model="cateForm.category1Id" @change="handler1" :disabled="show">
                    <el-option :label="category1.name" :value="category1.id" v-for="(category1, index) in this.list1"
                        :key="category1.id"></el-option>
                </el-select>
            </el-form-item>

            <el-form-item label="二级分类">
                <el-select placeholder="请选择" v-model="cateForm.category2Id" @change="handler2" :disabled="show">
                    <el-option :label="category2.name" :value="category2.id" v-for="(category2, index) in list2"
                        :key="category2.id"></el-option>
                </el-select>
            </el-form-item>

            <el-form-item label="三级分类">
                <el-select placeholder="请选择" v-model="cateForm.category3Id" @change="handler3" :disabled="show">
                    <el-option :label="category3.name" :value="category3.id" v-for="(category3, index) in list3"
